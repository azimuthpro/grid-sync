...
export default function InsolationPage() {
  ...
  const [accumulatedTableData, setAccumulatedTableData] = useState<
    InsolationData[]
  >([]);
  ...
  const tableFilters = useMemo(() => {
    ...
    switch (tableTimeRange) {
      case '24h':
        ...
        break;
      case '3d':
        startDate = new Date(referenceDate.getTime() - 3 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split('T')[0];
        break;
      case '7d':
  ...
  useEffect(() => {
    ...
    if (
      shouldResetData.current ||
      (page === 1 && accumulatedTableData.length === 0)
    ) {
      ...
      setAccumulatedTableData((prev) => {
        ...
        const existingIds = new Set(
          prev.map((item) => `${item.date}-${item.hour}-${item.city}`)
        );
        const newData = tableData.filter(
          (item) => !existingIds.has(`${item.date}-${item.hour}-${item.city}`)
        );
  ...
  return (
    <div className="p-8">
      ...
      {/* Error message */}
      {error && (
        <div className="mb-8 bg-red-950/50  rounded-md p-4">
      ...
      {isLoading && (
        <>
          <div className="mb-8 bg-gray-900 rounded-lg  p-8">
      ...
      {!error && (chartData.length > 0 || accumulatedTableData.length > 0) && (
        <>
          ...
          {!isLoading && (
            <InsolationDataTable
              data={
                accumulatedTableData.length > 0
                  ? accumulatedTableData
                  : tableData || []
              }
        ...
        accumulatedTableData.length === 0 && (
          <div className="bg-gray-900 rounded-lg border border-gray-800 p-12 text-center">
            ...
            <p className="text-gray-400 mb-6">
              Nie znaleziono danych dla wybranych filtrów. Spróbuj zmienić
