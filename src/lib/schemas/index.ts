...
export const locationSchema = z.object({
  ...
  user_id: z.string(),
  name: z.string().min(1, 'Nazwa lokalizacji jest wymagana').max(50, 'Nazwa nie może być dłuższa niż 50 znaków'),
...
export const consumptionProfileSchema = z.object({
  ...
  day_of_week: z.number().or(z.string().pipe(z.coerce.number())).pipe(
    z.number().int().min(0, 'Dzień tygodnia musi być między 0-6').max(6, 'Dzień tygodnia musi być między 0-6')
  ),
  hour: z.number().or(z.string().pipe(z.coerce.number())).pipe(
    z.number().int().min(0, 'Godzina musi być między 0-23').max(23, 'Godzina musi być między 0-23')
  ),
  consumption_kwh: z.number().or(z.string().pipe(z.coerce.number())).pipe(
    z.number().nonnegative('Zużycie energii nie może być ujemne').max(100, 'Zużycie energii nie może być większe niż 100 kWh')
  ),
  created_at: z.string().optional(),
  updated_at: z.string().optional()
})
...
export const chatMessageSchema = z.object({
  ...
  content: z.string().min(1, 'Wiadomość nie może być pusta').max(1000, 'Wiadomość jest zbyt długa'),
  timestamp: z.date().default(() => new Date())
})

export const chatRequestSchema = z.object({
  message: z.string().min(1, 'Wiadomość jest wymagana').max(1000, 'Wiadomość jest zbyt długa'),
...
export type CreateConsumptionProfileSchema = z.infer<typeof createConsumptionProfileSchema>
export type BatchConsumptionUpdateSchema = z.infer<typeof batchConsumptionUpdateSchema>
export type InsolationDataSchema = z.infer<typeof insolationDataSchema>
export type CreateInsolationDataSchema = z.infer<typeof createInsolationDataSchema>
